import { CalendarDaysIcon, GiftIcon, HeartIcon, PhoneIcon } from '@heroicons/react/24/outline';
import { BUSINESS_NAME, FACEBOOK_URL, WHATSAPP_NUMBER } from '@/data/products';

export default function Footer() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="bg-gray-900 text-white py-10 md:py-12">
      <div className="container mx-auto px-4">
        <div className="max-w-5xl mx-auto">
          {/* Brand */}
          <div className="text-center mb-6 md:mb-8">
            <h3 className="text-2xl md:text-3xl font-display font-bold mb-3 flex items-center justify-center gap-2">
              <HeartIcon className="w-6 h-6 md:w-8 md:h-8 text-rose-400" />
              {BUSINESS_NAME}
            </h3>
            <p className="text-gray-400 text-base md:text-lg mb-4">
              Detalles artesanales hechos con amor
            </p>
            
            {/* Social Links */}
            <div className="flex flex-col sm:flex-row items-center justify-center gap-3 md:gap-4">
              <a
                href={FACEBOOK_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-5 md:px-6 py-2.5 md:py-3 rounded-full font-semibold transition-all transform hover:scale-105 shadow-lg text-sm md:text-base w-full sm:w-auto justify-center"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
                </svg>
                Síguenos en Facebook
              </a>
              <a
                href={`https://wa.me/${WHATSAPP_NUMBER}?text=Hola%0AQuiero mas informacion sobre ${encodeURIComponent(BUSINESS_NAME)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 bg-green-600 hover:bg-green-700 text-white px-5 md:px-6 py-2.5 md:py-3 rounded-full font-semibold transition-all transform hover:scale-105 shadow-lg text-sm md:text-base w-full sm:w-auto justify-center"
              >
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/>
                </svg>
                WhatsApp
              </a>
            </div>
          </div>

          {/* Divider */}
          <div className="h-px bg-gray-700 mb-6 md:mb-8"></div>

          {/* Info Grid */}
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 md:gap-8 mb-6 md:mb-8 text-center">
            <div className="flex flex-col items-center gap-2">
              <CalendarDaysIcon className="w-7 h-7 md:w-8 md:h-8 text-rose-400" />
              <p className="text-gray-400 text-sm">Fecha límite de pedidos</p>
              <p className="font-semibold text-base md:text-lg">12 de febrero {currentYear}</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              <GiftIcon className="w-7 h-7 md:w-8 md:h-8 text-rose-400" />
              <p className="text-gray-400 text-sm">Modalidad</p>
              <p className="font-semibold text-base md:text-lg">Bajo pedido personalizado</p>
            </div>
            <div className="flex flex-col items-center gap-2">
              <HeartIcon className="w-7 h-7 md:w-8 md:h-8 text-rose-400" />
              <p className="text-gray-400 text-sm">Campaña</p>
              <p className="font-semibold text-base md:text-lg">San Valentín {currentYear}</p>
            </div>
          </div>

          {/* Contact Info */}
          <div className="text-center mb-6 md:mb-8">
            <p className="text-gray-400 text-sm mb-3 flex items-center justify-center gap-2">
              <PhoneIcon className="w-4 h-4" />
              Contáctanos por WhatsApp
            </p>
            <p className="text-rose-400 font-semibold text-lg mb-4">
              +51 973 175 966
            </p>
            <div className="flex items-center justify-center gap-2 text-sm text-gray-400">
              <span>Síguenos:</span>
              <a
                href={FACEBOOK_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="text-blue-400 hover:text-blue-300 underline transition-colors"
              >
                @amorendetalles.pe
              </a>
            </div>
          </div>

          {/* Divider */}
          <div className="h-px bg-gray-700 mb-6"></div>

          {/* Copyright */}
          <div className="text-center">
            <p className="text-gray-500 text-xs md:text-sm flex items-center justify-center gap-2 flex-wrap">
              © {currentYear} {BUSINESS_NAME}. Todos los derechos reservados. 
              <span className="flex items-center gap-1">
                Hecho con <HeartIcon className="w-4 h-4 text-rose-400 inline" /> para celebrar el amor.
              </span>
            </p>
          </div>
        </div>
      </div>
    </footer>
  );
}
